...
<nav class="fixed bottom-0 left-0 right-0 z-50 border-t bg-background md:hidden">
    <div class="flex items-center justify-around h-16">
        {#each tabs as tab}
            <Link
                href={tab.href}
                class="flex flex-col items-center justify-center gap-1 px-3 py-2 text-xs font-medium transition-colors {url.isCurrentOrParentUrl(tab.href, url.currentUrl) ? 'text-primary' : 'text-muted-foreground'}"
            >
